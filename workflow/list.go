...
func listAction(ctx context.Context, c *cli.Command) error {
	...
	if err := kcli.List(ctx, list, listOpts...); err != nil {
		cliutil.EmitStatus(os.Stderr, fmt.Errorf("list SeiNodeTaskWorkflows: %w", err))
		return cli.Exit("", 1)
	}
...
var listCmd = cli.Command{
	Name:  "list",
	Usage: "List SeiNodeTaskWorkflows",
	Description: "Prints a List of native SeiNodeTaskWorkflow CRs with full " +
		"status, including each .items[].status.phase and .status.plan " +
		"progression.",
	Flags: []cli.Flag{
		&cli.StringFlag{
			Name:    "namespace",
