...
func getAction(ctx context.Context, c *cli.Command) error {
	...
	if name == "" {
		cliutil.EmitStatus(os.Stderr, cliutil.UsageError("name argument required: seictl network get <name>"))
		return cli.Exit("", 1)
	}
	...
	if err := kcli.Get(ctx, client.ObjectKey{Namespace: ns, Name: name}, obj); err != nil {
		cliutil.EmitStatus(os.Stderr, fmt.Errorf("get SeiNetwork %s/%s: %w", ns, name, err))
		return cli.Exit("", 1)
	}
...
var getCmd = cli.Command{
	Name:      "get",
	Usage:     "Read a SeiNetwork by name",
	ArgsUsage: "<name>",
